...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	then
		...
		then
			...
			for w in $curr
			do
				if [[ ( "$i" -eq 2 ) && ( "$c" -ge 2 && "$c" -le 10 ) ]]; then
					w=`echo "$w+$d" | bc`
				elif [[ ( "$i" -eq 2 ) && ( "$c" -ge 11 && "$c" -le 19 ) ]]; then
					w=`echo "$w-$d" | bc`
				elif [[ ( "$i" -eq 3 ) && ( "$c" -ge 2 && "$c" -le 10 ) ]]; then
                                        w=`echo "$w+$d" | bc`
				elif [[ ( "$i" -eq 3 ) && ( "$c" -ge 20 && "$c" -le 28 ) ]]; then
                                        w=`echo "$w-$d" | bc`
				elif [[ ( "$i" -eq 4 ) && ( "$c" -ge 11 && "$c" -le 19 ) ]]; then
                                        w=`echo "$w+$d" | bc`
				elif [[ ( "$i" -eq 4 ) && ( "$c" -ge 20 && "$c" -le 28 ) ]]; then
                                        w=`echo "$w-$d" | bc`
				fi
				new="$new${w}     "
				i=$(($i+1))
