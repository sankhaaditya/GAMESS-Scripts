...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	then
		...
		then
			...
			for w in $curr
			do
				if [[ ( "$i" -eq 2 ) && ( "$c" -eq 4 || "$c" -eq 14 ) ]]; then
					w=`echo "$w+$d" | bc`
				elif [[ ( "$i" -eq 2 ) && ( "$c" -eq 5 || "$c" -eq 13 ) ]]; then
					w=`echo "$w-$d" | bc`
				elif [[ ( "$i" -eq 3 ) && ( "$c" -eq 5 || "$c" -eq 23 ) ]]; then
                                        w=`echo "$w+$d" | bc`
				elif [[ ( "$i" -eq 3 ) && ( "$c" -eq 4 || "$c" -eq 22 ) ]]; then
                                        w=`echo "$w-$d" | bc`
				elif [[ ( "$i" -eq 4 ) && ( "$c" -eq 14 || "$c" -eq 22 ) ]]; then
                                        w=`echo "$w+$d" | bc`
				elif [[ ( "$i" -eq 4 ) && ( "$c" -eq 13 || "$c" -eq 23 ) ]]; then
                                        w=`echo "$w-$d" | bc`
				fi
				new="$new${w}     "
				i=$(($i+1))
